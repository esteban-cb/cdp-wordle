...
export async function createCDPWallet(username: string): Promise<WalletInfo> {
  try {
    console.log(
      `Attempting to get or create CDP wallet for account: ${username}`
    );
...
): Promise<{ balance: string; usdcBalance: string }> {
  try {
    // Fix the URL by using absolute path with origin
    const origin = window.location.origin;
    const response = await fetch(`${origin}/api/wallet`, {
...
export async function getBalance(
  ...
  try {
    const balances = await getWalletBalance(address, network);
